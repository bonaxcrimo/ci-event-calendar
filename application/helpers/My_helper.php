...
function alert($type,$message){
    $html = "<div class='alert alert-".$type."'><a href='#'' class='close' data-dismiss='alert'>&times;</a>".$message."
    </div>";
    return $html;
}
function print_recursive_list($data)
{
    ...
    foreach($data as $list)
    {
        ...
        $menuexe = $list['link']!=''?$list['link']:$menuexe;
        if ($list['menuexe'] !='#'){
            $target = $data['menuname']=='Calendar'?'_blank':'';
            $str .= "<li class='dropdown ".$list['menuicon']."'><a href='".$menuexe."' target='".$target."'>".$list['menuname']."</a>";
        }else{
...
function print_recursive_sidebar($data){
    $str = "";
    foreach($data as $list)
    {
        $menuexe = $list['menuexe']=="0"?"#":base_url().$list['menuexe'];
        ...
        $subchild = print_recursive_list($list['child']);
        if ($subchild==""){
            $target = $list['menuname']=='Calendar'?'_blank':'';
            $str .= "<li><a href='".$menuexe."' target='$target'><i class='".$list['menuicon']."' ></i>".$list['menuname']."</a></li>";
        }else{
            $str .= "<li class='treeview'>";
            $str .= "<a ><i class='".$list['menuicon']."'></i>".$list['menuname']."<span class='pull-right-container'><i class='fa fa-angle-left pull-right'></i></span></a>";
            $str .="<ul class='treeview-menu'>".$subchild."</ul></li>";
        }
...
function excel($nama_session,$tabel,$route){
    ...
    DATE_FORMAT($tabel.modifiedon,'%d-%m-%Y') modifiedon
    FROM $tabel inner join tblmember on $tabel.member_key = tblmember.member_key " . $where . " ORDER BY $sidx $sord");
    $CI->load->view($route,$data);
}
function getDataPeriodly($period='1',$table,$field,$sfield,$sort){
    $db = get_instance()->db;
    $where = ' where ';
    if($period=='1'){
...
function getParameterDefault($tipe){
    $db = get_instance()->db;
    if($tipe=='ACTIVITY'){
        $db->where('parametermemo',"DEFAULT")
        ->group_start() // Open bracket
        ->where('parametergrpid',$tipe)
        ->or_where('parametergrpid','SERVING')
        ->group_end(); // Close bracket
        $db->order_by("parameterid", "asc");
    }else{
        $db->where('parametergrpid',$tipe)
        ->where('parametermemo','DEFAULT');
        $db->order_by("parameterid", "asc");
    }
    $sql =$db->get('tblparameter');
    return $sql->result();
...
function getComboGrid($tipe){
    $data="<option value=''>Semua</option>";
    $sql = getParameter($tipe);
    foreach ($sql as $key) {
        // $data .=$key->parameter_key.":".$key->parametertext.";";
        $data .= "<option value='".$key->parameter_key."'>".$key->parametertext."</option>";
    }
    return $data;
}
